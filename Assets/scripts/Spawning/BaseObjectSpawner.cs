...
public abstract class BaseObjectSpawner : MonoBehaviour
{
    ...
    [SerializeField]
    protected MazeDestructibleSet destructibleSet;

    [Header("Sorting Layer Settings")]
    [SerializeField]
    protected string targetSortingLayer = "BreakableObjects";

    [SerializeField]
    protected int targetSortingOrder = 100;

    [Header("Hierarchy Container")]
    [SerializeField]
    protected Transform gameplayObjectsContainer;

    protected HashSet<Vector3Int> occupiedCells = new HashSet<Vector3Int>();
    protected Dictionary<MazeDestructiblePattern, int> patternSpawnCounts =
        ...
        int seedOverride = -1)
    {
        ...
        {
            Debug.LogWarning("BaseObjectSpawner: No MazeDestructibleSet assigned or pattern list is empty.", this);
            return;
        }
        ...
        Debug.Log($"{gameObject.name} ({GetType().Name}): ¡ÉXITO! Se instanciaron {totalSpawned} objetos destruibles sobre las celdas de camino del laberinto.", this);
    }
    ...
    protected System.Random InitializeRandomGenerator(int seedOverride)
    {
        if (seedOverride >= 0)
        {
    ...
    protected void EnsureMinimumPatternAmounts(
        MazeCellType[,] mazeMatrix,
        Vector3Int tilemapOrigin,
        Vector2Int logicalCellTileSize,
        Vector3Int playerCellPosition,
    ...
    protected MazeDestructiblePattern SelectPatternWeighted(System.Random random)
    {
    ...
    protected bool TrySpawnPatternAtRandomPosition(
    ...
    protected bool ValidateRegionForPattern(
        MazeDestructiblePattern pattern,
    ...
    protected List<Vector3Int> GetCellsInPatternRegion(Vector3Int originCell, int width, int height)
    {
    ...
    protected void OnDestructibleDestroyed(DestructibleObject destructible)
    {
